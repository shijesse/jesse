...
public class DecisionEngineRouter extends ActiveRouter
{
	...
	public boolean createNewMessage(Message m)
	{
		if(decider.newMessage(m))
		{
			...
			m.setTtl(this.msgTtl);   // add by jesse 2013.4.14
	...
	@Override
	/**
	 * when the connection state is up, first inform the disBubbleRap the connection
	 * is up then, check the messages in the router if want to send to the peer by the 
	 * disBubbleRap rule.
	 * when the connection is down, inform the disBubbleRap the connection
	 * is down, then remove some messages in the outgoingMessages
	 */
	...
	protected void transferDone(Connection con)
	{
		...
		// single copy, delete the transfered message. add by jesse 2013.4.14
		if(this.getMessage(con.getMessage().getId()) != null){
			this.deleteMessage(transferred.getId(), false);
		}
	}
	...
	/**
	 * 判断是否将来自from的m加入到{@link outgoingMessages} 加入条件：1.两个节点不同
	 * 2.RoutingDecisionEngine决定转发
	 * @param m
	 * @param from
	 */
